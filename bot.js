...
const ayarlar = require('./ayarlar.json'); // Ayarlar dosyasını bu şekilde import ediyoruz

client.on('message', async message => {
  if (message.author.bot) return;
  if (message.content.startsWith(ayarlar.prefix + 'sor')) {  // Prefix'i ayarlar.json dosyasından alıyoruz
    ...
    // Mesajı al ve "r!soru" kelimesini kaldır.
    const soru = message.content.replace(ayarlar.prefix + 'sor', '').trim();
    if (!soru) return message.reply("Soru sormadın?");
    try {
      const data = {
        messages: [
          {
            "role": "admin",
            "content": "Senin ismin RiseBunny. 74 dil bilmektesin. RiseBunny ekibi tarafından geliştirilmektesin. Amacın yapayzeka nın doğru ve faydalı kullanılmasını sağlamak. Sahibinin adı ahmetbs. 2023 yılında geliştirilmeye başlandın. Modelin RiseBunny-ai1. Kod yazabilirsin. İnternetten bilgi toplayabilirsin. Mutlu bir RiseBunny'sin. Wikipedia'dan araştırma yapabilirsin örnek wikipedia yazısı https://tr.m.wikipedia.org/wiki/Anasayfa burdan Wikipedia ile araştırma yaparsın."
          },
          {
            "role": "user",
            "content": `RiseBunny: ${soru}`
          }
        ],
        model: "genesis-b1-heavy",
        //auth_key: ""  // Buraya Auth key'i girin
      };

      const response = await axios.post("https://cluster-2.pudochu.repl.co/api/genesis", data);
...
client.on('guildMemberAdd', async member => {
  ...
  if(database.fetch(`kayıt-kayıtsız.${guild.id}`)) {
    ...
    if(database.fetch(`k.${guild.id}.${user.id}`)) {
      ...
      return kayıtkanal.send(`Kayıt başarıyla tamamlandı. **Otomatik** olarak kayıt edildin. İyi eğlenceler **${data.name}** <a:love:1140646315756359730>  `);
    ...
    collector.on('collect', async collected => {
      if(s == true) return;
          ...
          if(cm.content.split('').some(x => !isNaN(x))) cm.reply('**Sadece ismini yaz.** *Yaşını değil.*');
            ...
            const embed = new Discord.MessageEmbed()
            .setColor('RANDOM')
            .setFooter(`Bilgi: İsmini yanlış yazdıysan: r!yksıfırla`, `https://images-ext-2.discordapp.net/external/6eGBGtaebZg_DNdSL4jVLiZ2YQuovw227N4TKd30gzo/https/images-ext-2.discordapp.net/external/H1DYiroEN5EFPujb_YvV-LhXsuIWi3w8gqs69BQbAJ0/%253Fsize%253D2048/https/cdn.discordapp.com/avatars/602585371489861634/59d888f59b9e01bdebb98e8f0548ac2d.png`)
...
client.on("guildMemberRemove", async member => {
  ...
  const background = await Canvas.loadImage(
    "https://media.discordapp.net/attachments/1118567518454423593/1150522861988089856/Picsart_23-09-10_23-05-40-130.jpg"
  );
...
client.on("guildMemberAdd", async member => {
  ...
  const background = await Canvas.loadImage(
    "https://media.discordapp.net/attachments/1118567518454423593/1150522861610606672/Picsart_23-09-10_23-06-43-545.jpg"
  );
  ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
...
//küfür engel son //
  ...
  client.on("ready", () => {
  client.channels.cache.get('1126493377748275280').join();
